        ...
        <div style={descriptionTextStyles}>
          <Text size={200} style={{ color: tokens.colorNeutralForeground2 }}>
            {(() => {
              switch (dataFormat) {
                case 'XYZ Triplets':
                  return 'Requires one X column, one Y column, and one Z column for 3D surface plotting.';
                case 'Many Z':
                  return 'Requires multiple Z columns. X and Y scales are automatically generated (10,20,30... and 1,2,3...).';
                case 'XY Many Z':
                  return 'Requires one X column, one Y column, and multiple Z columns. Uses selected X/Y variables with multiple Z surfaces.';
                default:
                  return '';
              }
            })()}
