...
export class MeshPlotValidationError extends Error {
  constructor(public errors: ValidationError[]) {
    super(`Mesh plot validation failed: ${errors.map(e => e.message).join(', ')}`);
    this.name = 'MeshPlotValidationError';
  }
}

/**
 * Validate 3D mesh plot requirements
 */
export function validateMeshPlotRequirements(
  dataFormat: string,
  selectedVariables: {
    x?: string[];
    y?: string[];
