import { FC } from 'react';
import { Field, Dropdown, Option } from '@fluentui/react-components';
import type { AreaSubType } from '../areaPlotSlice';

/**
 * Props for the ProjectAndType component
 */
interface ProjectAndTypeProps {
    classes: Record<string, string>;
    projects: string[];
    selectedProject: string | null;
    setProject: (p: string) => void;
    subType: AreaSubType | null;
    setSubType: (t: AreaSubType) => void;
    subTypes: AreaSubType[];
}

/**
 * Component for selecting project and area plot type
 */
export const ProjectAndType: FC<ProjectAndTypeProps> = ({
    classes,
    projects,
    selectedProject,
    setProject,
    subType,
    setSubType,
    subTypes,
}) => {
    return (
        <div className={classes.sectionCard}>
            <Field label="Project" required>
                <style>{`
          .projectDropdown .fui-Listbox { max-height: 216px; overflow-y: auto; }
        `}</style>
                <Dropdown
                    placeholder="Choose a project..."
                    value={selectedProject ?? ''}
                    onOptionSelect={(_, data) => setProject(data.optionValue ?? '')}
                    className={`${classes.dropdown} projectDropdown`}
                >
                    {projects.map((project) => (
                        <Option key={project} value={project} text={project}>
                            {project}
                        </Option>
                    ))}
                </Dropdown>
            </Field>

            <Field label="Area Plot Type" required>
                <Dropdown
                    placeholder="Select plot type..."
                    value={subType ?? ''}
                    onOptionSelect={(_, data) => setSubType(data.optionValue as AreaSubType)}
                    className={classes.dropdown}
                >
                    {subTypes.map((type) => (
                        <Option key={type} value={type}>
                            {type}
                        </Option>
                    ))}
                </Dropdown>
            </Field>
        </div>
    );
};
