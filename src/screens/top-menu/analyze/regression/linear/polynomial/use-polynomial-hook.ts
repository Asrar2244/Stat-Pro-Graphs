...
interface IEstimate {
  confidence: string;
  degree: string;
  polynomialType: 'natural' | 'orthogonal';
  inference: boolean;
}

interface IPolynomialStore {
  model: IModel;
  estimate: IEstimate;
...
const initialEstimate: IEstimate = {
  confidence: '0.95',
  degree: '2', // Reduced from 3 to 2 to prevent backend errors
  polynomialType: 'natural',
  inference: true,
};

export const usePolynomial = create<IPolynomialStore>((set, get) => ({
  model: initialModel,
  ...
  get isAnalyzeDisabled() {
    const { model } = get();
    return model.dependentList.size === 0 || model.independentList.size !== 1;
  },
}));
