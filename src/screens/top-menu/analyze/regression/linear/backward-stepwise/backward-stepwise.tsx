...
import { useLinearLeastSquares } from '../forward-stepwise/use-squares-hook';
...
const BackwardStepwiseComponent: FC<IModal> = ({ ...props }) => {
  const { t } = useTranslation('regLinearBackwardStepwise');
  ...
  const { executeAnalysis } = usePrepareAnalysis({
    config,
    columns,
    queueFor: t('title', { ns: 'regLinearBackwardStepwise' }),
    queueType: 'regLinearBackwardStepwise',
  });
  ...
  return (
    <Modal
      ...
      title={t('title', { ns: 'regLinearBackwardStepwise' })}
      size="medium"
...
export { BackwardStepwiseComponent };
