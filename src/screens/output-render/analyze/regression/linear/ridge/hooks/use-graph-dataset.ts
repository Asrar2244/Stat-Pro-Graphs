...
export const useGraphDataset = (parameters: IUseGraphDataset, tableRef: any): any[] => {
  ...
  useEffect(() => {
    if (parameters.params && parameters.params?.id > 0) {
      ...
      const xKey = tableRef.current?.dependent_var_names[0];

      for (
        let dependent = 0;
        dependent < tableRef.current?.independent_var_names.length;
        dependent++
      ) {
        const yKey = tableRef.current?.independent_var_names[dependent];
        const x: number[] = [];
        const y: number[] = [];
