...
export const assessDataQuality = (
  ...
  // Use iterative approach instead of spread operator to handle large datasets
  const xRange = xValues.length > 0
    ? xValues.reduce((acc, val) => ({ min: Math.min(acc.min, val), max: Math.max(acc.max, val) }),
      { min: xValues[0], max: xValues[0] }).max -
    xValues.reduce((acc, val) => ({ min: Math.min(acc.min, val), max: Math.max(acc.max, val) }),
      { min: xValues[0], max: xValues[0] }).min
    : 0;

  const yRange = yValues.length > 0
    ? yValues.reduce((acc, val) => ({ min: Math.min(acc.min, val), max: Math.max(acc.max, val) }),
      { min: yValues[0], max: yValues[0] }).max -
    yValues.reduce((acc, val) => ({ min: Math.min(acc.min, val), max: Math.max(acc.max, val) }),
      { min: yValues[0], max: yValues[0] }).min
    : 0;
